...
void CaesarMessage::encode(const char* _message, int _shift) {
	...
	Just encode the alphabet, ignore any others */
     int j  =0;
     for (int i = 0;;i++) {
     if (_message[i] == 0) break;
     if (_message[i] >= 65 && _message[i] <= 90)       textBuffer[j] = _message[i] + 32;
     else if (_message[i] >= 97 && _message[i] <= 122)
     textBuffer[j] = _message[i];    
     else continue;    
     textBuffer[j] += _shift;
     if (textBuffer[j] > 122) textBuffer[j] -= 25+1;
    j++;
  }  
  textBuffer[j] = 0;
//cout << textBuffer<<'\n';
...
void CaesarMessage::decode(int _shiftKey, char*& _textContainer) {
    _textContainer = new char[9999];
  for (int i = 0;  textBuffer[i] != 0 ;i++) {
    _textContainer[i] = textBuffer[i]-_shiftKey;
    if (_textContainer[i] < 97) _textContainer[i] += 25 + 1;
  }
  //cout << _textContainer<<'\n';
}
